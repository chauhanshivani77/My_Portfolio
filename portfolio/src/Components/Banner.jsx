import React, { useEffect, useState } from "react";
import BannerCon from "../css/Banner.styled";
import Typewriter from "typewriter-effect";
import { BsLinkedin, BsGithub } from "react-icons/bs";
import Avatar1 from "../assets/image/profilePic.png";
import Avatar2 from "../assets/image/profilePic-2.jpg";

import { SiGmail } from "react-icons/si";

const Banner = () => {
  return (
    <BannerCon>
      <div className="bannerContainer">
        <div className="typeWirter">
          <div className="introMyself">
            <p className="heading">Hi, I am Shivani Chauhan,</p>
            <div className="subHeading">
              <Typewriter
                options={{
                  strings: [
                    "Full Stack Web Developer !",
                    "Frontend Developer !",
                    "Backend Developer !",
                    "MERN Stack Developer",
                    "A tech enthusiast !",
                  ],
                  autoStart: true,
                  loop: true,
                  delay: 200,
                }}
              />
            </div>
            <p className="tagLine">
              I am Full Stack Web Developer and i have knowledge in HTML, CSS, JS, REACT and i make 
              user-friendly websites.
            </p>
            <div className="typeButtons">
              <button className="learn-more">
                <a
                  href={
                    "https://drive.google.com/file/d/1qAiS9fEl6Eya4Z29_UpHI9DiNq1qtdGy/view?usp=share_link"
                  }
                  target={"_blank"}
                >
                  <span className="circle" aria-hidden="true">
                    <span className="icon arrow"></span>
                  </span>
                  <span className="button-text">Resume</span>
                </a>
              </button>
            </div>
            <div className="socialIconContainer">
              <a
                href="https://www.linkedin.com/in/shivani-chauhan-67b482239"
                target={"_blank"}
              >
                <BsLinkedin className="socialIcons" />
              </a>
              <a href="https://github.com/chauhanshivani77" target={"_blank"}>
                <BsGithub className="socialIcons" />
              </a>
              <a href="mailto:chauhan2002shiv@gmail.com" target={"_blank"}>
                <SiGmail className="socialIcons" />
              </a>
            </div>
          </div>
        </div>

        <div className="imageContainer">
          <img
            src={Avatar2}
            alt=""
            className="avatar"
          />
        </div>
      </div>
    </BannerCon>
  );
};

export default Banner;
